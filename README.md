...
`npm run build:gh-pages` builds the full site into `build/website/`
(gitignored):

- `/` — renders the examples of any spec, live, in the browser. Visiting it
  with no query redirects to `?url=<default spec URL>` (the CLI's default,
  `https://w3c-ccg.github.io/zcap-spec/v0.4.0-draft/`); the page then fetches
  that URL, extracts its examples, and renders them as they stream in. Any
...
- `/diff/` (experimental) — compares the examples of two versions of a spec,
  ...
  (the default when `?from=` and `?to=` are absent). Each example is shown as
  changed (with a line diff), added, removed, or unchanged. Examples are
  matched by content rather than by number, because ReSpec numbers examples by
  position and inserting one renumbers every example after it: pairs are
  scored by the words they share (plus a shared JSON `id`), and aligned in
  document order; similar leftovers are marked as moved. `?view=parsed`
  compares JSON with comments removed and formatting normalized,
  `?whitespace=ignore` ignores whitespace like `git diff -w` and also line
  breaks (so reflowed JSONC with its comments still intact reads as unchanged),
  `?unchanged=show` lists unchanged examples, and `?context=full` shows every
  line. The logic is `website/diff.ts`; the page script is
  `website/diff-app.ts`, compiled into `/app/`.
- `/examples/` — every example in the bundled spec snapshot (`fixtures.ts`),
  described from its own parsed content, generated by `website/build.ts`
- `/lib/` — `examples.js` and `fixtures.js` as browser ES modules (with
  `.d.ts`), so any web page can use the library without installing it:
  ...
  Only the modules with no Node.js dependencies are hosted; the package entry
  point re-exports the Node CLI wiring and cannot load in a browser.
- `/docs/` — the TypeDoc API docs

To preview exactly what will be published:

```shell
npm run dev   # alias for dev:gh-pages
# Serving build/website at http://127.0.0.1:4000/zcap-spec-examples/
```

`npm run dev` runs `build:gh-pages` (the same builds `gh-pages.yml` runs, into
the same `build/website/`) and serves it with `etc/serve-gh-pages.ts`, which
mimics GitHub Pages: the site lives under `/zcap-spec-examples/` (so a
root-absolute link breaks locally just as it would in production), directories
redirect to a trailing slash, and `404.html` is used when present. `--port`
(or `PORT`) and `--base` are accepted after `--`. It does not watch for
changes; rerun it after editing.

It is published to <https://gobengo.github.io/zcap-spec-examples/> by
`.github/workflows/gh-pages.yml` on every push to `main`. That workflow is the
only one that uploads to Pages. It assembles the site from two reusable
composite actions, each of which just builds into an `output-directory` and
knows nothing about Pages:

- `.github/actions/build-website` — `website/`, `/examples/`, and `/lib/`
- `.github/actions/build-docs` — type-check, then TypeDoc

`.github/workflows/docs.yml` uses `build-docs` to check the docs build on pushes
and pull requests, without publishing.

Nothing generated is ever committed and there is no `gh-pages` branch, which is
why `docs/` and `build/` stay in `.gitignore`. Only first-party `actions/*`
steps are used.
